...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% INPUTS
addpath('/home/nando/MuRAT5_3_6/Utilities_Matlab')
clear
close all
clc
...
if evst==2
    ...
    if degorutm==111
        load coastlines
        hold on
        geoshow(coastlat,coastlon, 'Color', 'black', 'LineWidth', 2.0);
        xlim([15.4 16.4]);ylim([39.6 40.4]);
    end
    if pa<3
...
Qcpd=figure('Name','Qc and peak-delays',...
   'NumberTitle','off','visible',visib);
subplot(2,1,1)
plot(r(retainQm),Qm(retainQm),'o',...  %funzione della dist epicent.
'MarkerSize',6,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[0 0 0])
%plot(time0(retainQm),Qm(retainQm),'o',...
   %'MarkerSize',6,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',[0 0 0])
hold on
%plot(r(discardQm),Qmdis2(discardQm),'o',...  %funz. della dist. epicent.
  % 'MarkerSize',6,'MarkerEdgeColor',[1 0 0],'MarkerFaceColor',[1 0 0])
plot(r(discardQm2),Qmdis(discardQm2),'o',...  %funz. della dist. epicent.
   'MarkerSize',6,'MarkerEdgeColor',[1 0 0],'MarkerFaceColor',[1 0 0])
%plot(time0(discardQm2),Qmdis(discardQm2),'o',...
  % 'MarkerSize',6,'MarkerEdgeColor',[1 0 0],'MarkerFaceColor',[1 0 0])
hold off
title('Dependence of inverse Qc on epicentral distance');
xlabel('Epicentral distance (km)','FontSize',12,'FontWeight','bold','Color','k')
ylabel('Inverse Qc','FontSize',12,...
   'FontWeight','bold','Color','k')

subplot(2,1,2)
plot(fitrobust,'k--',l10l,l10pd,'ko',outlierspd,'ro')
title('Dependence of peak delays on epicentral distance');
xlabel('Log. Epicentral Distance (km)','FontSize',12,'FontWeight','bold','Color','k')
...
geoshow(faglie, 'Color', 'black', 'LineWidth', 1.0)
hold on 
faglieIthaca = shaperead('Faglie_LatLon.shp', 'UseGeoCoord', true)
geoshow(faglieIthaca, 'Color', 'black', 'LineWidth', 1.0)
axis equal
view(2)
hcb=colormap(autumn);
colorbar
...
hold on
scatter(staz(:,1),staz(:,2),sz,'^','MarkerEdgeColor',...
    [1 1 1], 'MarkerFaceColor',[0 0 1], 'LineWidth',1)
if degorutm==111
    hold on
    geoshow(coastlat,coastlon, 'Color', 'black', 'LineWidth', 2.0);
    xlim([15.4 16.4]);ylim([39.6 40.4]);
...
elseif pa>=2
    ...
    Qccheck=figure('Name','Qc checkerboard test','NumberTitle',...
        'off','visible',visib);
    %faccio due plot, anzichè un unico subplot
    %subplot(1,2,1)
    contourf(X,Y,QQchi');
    axis equal
    view(2)
    ...
    hold off
    FName = 'Qc_checkerboard_in';
    saveas(Qccheck,fullfile(FPath, FLabel, FName), fformat);
    
    
    Qccheckout=figure('Name','Qc checkerboard test','NumberTitle',...
        'off','visible',visib);
    %subplot(1,2,2)
    contourf(X,Y,QQcho');
    axis equal
    ...
    FName = 'Qc_checkerboard_out';
    saveas(Qccheckout,fullfile(FPath, FLabel, FName), fformat);
...
miQcm=min(Qcef(:,4));   %minimo Qc nel graf Parameter space plot
maQcm=max(Qcef(:,4));   %massimo Qc nel graf Parameter space plot 
mipdm=min(pdef(:,4));   %minimo PD nel graf Parameter space plot
mapdm=max(pdef(:,4));   %massimo PD nel graf Parameter space plot
trepd=0.05*mapdm;       %fattore moltiplicativo da usare
treQc=0.05*maQcm;       %fattore moltiplicativo da usare

Qps=Qcef(:,4);
pdps=pdef(:,4);

%Grafico "Parameter space plot"
...
xlabel('inverse Qc','FontSize',12,'FontWeight','bold','Color','k')
...
%Grafico "parameter separation map"
mparam=figure('Name','Parameter separation map',...
    'NumberTitle','off','visible',visib);
surf(X,Y,param', 'EdgeColor', 'none');
view(2)
faglie = shaperead('FagliePollino.shp', 'UseGeoCoord', true)
geoshow(faglie, 'Color', 'black', 'LineWidth', 1.0)
hold on
...
fu=find(un_X==-1);
if isempty(fu)
    cmap = [0.9 0.9 0.9;  0 0.8 0; 0 0.6 1; 1 0.6 0];
    HTick={'Average','Ls La','Hs La','Ls Ha'};
else
    cmap = [0.9 0.9 0.9;  0 0.8 0; 0 0.6 1; 1 0.6 0; 1 0 0];
